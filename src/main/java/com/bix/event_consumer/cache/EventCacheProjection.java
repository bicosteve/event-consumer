...
/*
 * A flattened, JSON-serializable snapshot of an Event, built for the Redis Cache.
 * Each nested projection mirrors one level of the domain model and knows how to
 * build itself via a small {@code from(...)} factory
 *
 * */
...
public record EventCacheProjection(
  ...
  public static EventCacheProjection from(Event event) {
    ...
    return new EventCacheProjection(
        ...
        event.getSportId(),
        event.getEventDate(),
        schedule == null ? null : schedule.getSeasonType(),
        schedule == null ? null : schedule.getSeasonYear(),
        schedule == null ? null : schedule.getEventName(),
        schedule == null ? null : schedule.getEventHeadline(),
        status(event.getScore()),
        tournament(event),
        event.getTeams() == null
            ? List.of()
            : event.getTeams().stream().map(TeamProjection::from).toList(),
        event.getMarkets() == null
            ? List.of()
            : event.getMarkets().stream().map(MarketProjection::from).toList(),
        ScoreProjection.from(event.getScore()));
  }

  private static Integer status(Score score) {
    return score == null || score.getEventStatus() == null
        ? null
        : score.getEventStatus().getCode();
  }

  private static String tournament(Event event) {
    if (event.getSportId() == null) {
      return null;
    }

    try {
      return TournamentName.getSportsName(event.getSportId()).getDisplayName();
    } catch (IllegalArgumentException e) {
      // Unknown sport id: cache the snapshot without a tournament rather than fail the whole
      // projection.
      return null;
    }
  }
  ...
  record MarketProjection(
    ...
    static MarketProjection from(Market market) {
      return new MarketProjection(
          ...
          market.getEventId(),
          market.getParticipants() == null
              ? List.of()
              : market.getParticipants().stream()
                  .map(participant -> ParticipantProjection.from(participant, market.getId()))
                  .toList());
    }
  }
